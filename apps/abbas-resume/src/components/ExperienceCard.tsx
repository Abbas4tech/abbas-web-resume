...
interface ExperienceCardProps extends Omit<JobExperience, "techStack" | "description"> {
  techStack: {
    title: string;
...
const ExperienceCard: FC<ExperienceCardProps> = memo(
  ({
    ...
    const remoteLabel = useMemo(() => (workedRemotely ? " - Remote" : ""), [workedRemotely]);
        ...
        <StepBody>
          ...
          <StepContent>
            <div className="flex flex-col justify-between md:flex-row">
              ...
              <div className="flex items-center gap-2">
                <Icon {...durationIcon} />
                {`${formattedStartDate} - ${currentlyWorking ? "Present" : formattedEndDate}`}
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Icon {...roleIcon} />
